...
1. **按模块归属**：core / mmdtransform / vsdxdoc / opcpkg / app 各司其职
...
| 2 | `testjson` | mmdtransform | jsonToDiagram（JSON→Diagram、枚举映射） | testjson（保留） |
| 3 | `testparser` | mmdtransform | Translator 单例编排 + 服务（崩溃/自动启动/无效路径）+ 图类型解析覆盖 | testengine + testnodeprocess + testvalidation |
...
| 8 | `teste2e` | app | Application 全链（16 样例批量转换）+ 输出验证 | testvsdx + testapi |
...
| JSON→Diagram | mmdtransform | `testjson` |
| 翻译/服务/图型覆盖 | mmdtransform | `testparser` |
